...
qDebug() << "new ControlWidget";
    ...
    //设置控制面板的位置
    control->setGeometry(movePoint.x() - 100, movePoint.y() + 6, 100, 25);
    control->show();
}
...
void Screen::SaveFilePath(QString msg){
    this -> filepath = msg;
}

QString Screen::ReadFilePath(){
...
void Screen::savePixmap()
{
    ...
    qDebug()<<"保存到了"<<filename;
    this -> SaveFilePath(filename);
    emit Screen::Pixmap_in_screen(filename);
    qDebug() << "emit Pixmap_in_screen";
}
