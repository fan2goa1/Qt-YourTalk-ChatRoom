...
QNChatMessage::QNChatMessage(QWidget *parent) : QWidget(parent)
{
    ...
    m_loadingMovie->setFileName(":/img/loading4.gif");//消息发送时的图标
...
void QNChatMessage::setText(QString text, QString time, QSize allSize, QNChatMessage::User_Type userType)//是否显示加载图标
{
    m_msg = text;
    m_userType = userType;
    m_time = time;
...
QSize QNChatMessage::fontRect(QString str, int a)//计算绘图所需坐标
{
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
...
QSize QNChatMessage::getRealString(QString src)//将输出的字符串转化为符合气泡框的尺寸的字符串
{
    ...
    if(nCount == 0) {
        ...
        QString value = src;
        if(nMaxWidth > m_textWidth) {//如果字符串的长度大于气泡框的最大长度
...
void QNChatMessage::paintDocument(QPaintEvent *event)
{
    Q_UNUSED(event);
    ...
    if(m_userType == User_Type::User_She) { // 用户
        ...
        painter.drawRoundedRect(m_kuangLeftRect.x()-1,m_kuangLeftRect.y()-1,300,100,4,4);
        //框
...
void QNChatMessage::paintEvent(QPaintEvent *, int a, int b, int c, int d, QString str)
{
    QPainter painter(this);
    QPixmap pix;
    pix.load(str);
    painter.drawPixmap(a, b, c, d, pix);
}
...
void QNChatMessage::paintEvent(QPaintEvent *event)
{
    ...
    if(m_userType == User_Type::User_She) { // 用户
        ...
        else if(flag==1)
        {
            ...
            option.setWrapMode(QTextOption::WrapAtWordBoundaryOrAnywhere);
            painter.setFont(this->font());
            QRect n_textLeftRect = QRect(m_textLeftRect.x()+100,m_textLeftRect.y(),m_textLeftRect.width()-110,m_textLeftRect.height());
            painter.drawText(n_textLeftRect, m_msg, option);
            QPaintEvent *event;
            paintEvent(event, m_kuangLeftRect.x()+15,m_kuangLeftRect.y()+15, 65, 65);
        }
        else if(flag==2)
        {
            QPaintEvent *event;
            QString path = "./"+m_msg;
            paintEvent(event, m_kuangLeftRect.x(),m_kuangLeftRect.y(), 350, 250,path);
        }
        else if(flag==3)
        {
            QPaintEvent *event;
            qDebug()<< m_kuangLeftRect.x() << m_kuangLeftRect.y();
            QString path = ":/new/prefix1/img/"+m_msg;
            paintEvent(event, m_kuangLeftRect.x(),m_kuangLeftRect.y(), 40,40,path);
        }
        ...
        else if(flag==1)
        {
            ...
            painter.setFont(this->font());
            qDebug()<< m_msg;
            QRect n_textRightRect = QRect(m_textRightRect.x()+100,m_textRightRect.y(),m_textRightRect.width()-110,m_textRightRect.height());
            qDebug()<< m_msg;
            painter.drawText(n_textRightRect, m_msg, option);
            QPaintEvent *event;
            paintEvent(event, m_kuangRightRect.x()+15,m_kuangRightRect.y()+15, 65, 65);
        }
        else if(flag==2)
        {
            QString str0 = "./"+m_msg;
            QPaintEvent *event;
            qDebug()<< m_kuangRightRect.x() << m_kuangRightRect.y();
            paintEvent(event, m_kuangRightRect.x(),m_kuangRightRect.y(), 350, 250,str0);
        }
        else if(flag==3)
        {
            QPaintEvent *event;
            ...
            qDebug()<< "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^";
            qDebug()<< path;
            paintEvent(event, m_kuangRightRect.x(),m_kuangRightRect.y(), 40,40,path);
        }
    }  else if(m_userType == User_Type::User_Time) { // 时间
        QPen penText;
        penText.setColor(QColor(153,153,153));
